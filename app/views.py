...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if request.user.is_authenticated:
            ...
            # Điều hướng hoặc chuyển đến trang thanh toán hoặc trang tiếp theo của quy trình thanh toán
...
def payment(request):
    if request.user.is_authenticated:
        ...
        shipping_address = ShippingAddress.objects.filter(customer=customer, order=order).first()
        
        if order and shipping_address:
            ...
            customer_phone=shipping_address.mobile,
            customer_address=shipping_address.address,
            total_price=total_price,
            )
            invoice.save()
            invoice.order_items.set(order_items)
    ...
    # Xử lý trường hợp không tìm thấy thông tin đơn hàng hoặc địa chỉ giao hàng
...
def detail(request):
    ...
    categories = Category.objects.filter(is_sub = False)
    active_category = request.GET.get('category','')
    context = { 'products':products ,'active_category':active_category,'categories':categories,'items' :items,'order': order ,'cartItems':cartItems,'user_not_login':user_not_login,'user_login':user_login}
